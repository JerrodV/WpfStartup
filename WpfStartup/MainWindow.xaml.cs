...
namespace WpfStartup
{
	...
	public partial class MainWindow : Window
	{
		...
		private void Window_Loaded_1(object sender, RoutedEventArgs e)
		{
            ...
            Database db = new Database(Properties.Settings.Default.Base_ConnectionString, true);
            List<dynamic> data = new List<dynamic>();
            using (db)
            {
                db.Parameters.Add(new SqlParameter() { ParameterName = "@ID", Value = 0 });
                data.AddRange(Database.GetData(db.Request("Parameter Name")));
            }
